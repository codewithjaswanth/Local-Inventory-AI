...
import { Sparkles, UserPlus, Lock, Mail, User, Phone } from 'lucide-react';
import { Button } from '@/components/ui/Button';
import { useAuth } from '@/hooks/useAuth';
import { UserRole } from '@/services/auth.service';

export default function SignupPage() {
  const { signUp, user, role: currentRole, isLoading: isAuthLoading, error: authError } = useAuth();
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    const success = await signUp(email, password, name, role, phone);
    setIsSubmitting(false);

    if (!success) {
      setErrorMsg(authError || 'Failed to create account. Please try again.');
      return;
    }
