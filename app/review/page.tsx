      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        <header className="h-16 bg-white/90 dark:bg-[#060B14]/90 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800/80 px-6 flex items-center justify-between sticky top-0 z-30 transition-colors">
          <div className="flex items-center space-x-2 text-xs font-mono font-semibold text-slate-500 dark:text-slate-400">
            ...
            <span className="text-slate-900 dark:text-white font-bold">AI Extractions Review Queue</span>
          </div>
        </header>
        ...
        <main className="p-6 sm:p-8 space-y-8 max-w-7xl mx-auto w-full">
          ...
          {queue.length === 0 ? (
            <div className="bg-white dark:bg-[#111827] p-12 rounded-3xl border border-slate-200 dark:border-slate-800 text-center space-y-3 max-w-md mx-auto shadow-lg">
              <ShieldCheck className="w-12 h-12 text-emerald-500 dark:text-emerald-400 mx-auto" />
              <h3 className="text-base font-extrabold text-slate-900 dark:text-white">All Extractions Approved!</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">No pending WhatsApp voice or photo extractions in queue.</p>
            </div>
          ) : (
