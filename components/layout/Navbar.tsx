...
import { Sparkles, Search, Menu, X, LogIn, LogOut, User, Store, Settings, ChevronDown } from 'lucide-react';
                  ...
                  {isProfileDropdownOpen && (
                    <>
                      ...
                      <motion.div
                        ...
                        className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden z-50 p-1"
                      >
                        <div className="px-3 py-2 border-b border-slate-100 dark:border-slate-800 mb-1">
                          <p className="text-xs font-semibold text-slate-900 dark:text-slate-100">{profile?.name || 'Account'}</p>
                          <p className="text-[10px] text-slate-500 truncate">{user.email}</p>
                        </div>
                        <a href={role === 'admin' ? '/admin' : role === 'shopkeeper' ? '/dashboard' : '/profile'} className="flex items-center space-x-2 px-3 py-2 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg transition-colors">
                          <User className="w-4 h-4" />
                          <span>Profile</span>
                        </a>
                        <a href="/settings" className="flex items-center space-x-2 px-3 py-2 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg transition-colors">
                        ...
                        <button
                          ...
                          className="w-full flex items-center space-x-2 px-3 py-2 text-sm text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-lg transition-colors"
                        >
