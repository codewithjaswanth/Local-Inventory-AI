      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          <nav aria-label="Main Navigation" className="hidden md:flex items-center space-x-1 lg:space-x-2 bg-slate-100/60 dark:bg-slate-800/60 p-1.5 rounded-full border border-slate-200/60 dark:border-slate-700/60 text-sm font-medium text-slate-600 dark:text-slate-300">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="px-4 py-1.5 rounded-full hover:text-slate-900 dark:hover:text-white hover:bg-white dark:hover:bg-slate-700 transition-all duration-200"
              >
                {link.name}
              </a>
            ))}
          ...
          <div className="hidden md:flex items-center space-x-3">
            ...
            {/* Sign In CTA */}
            <Button
              onClick={() => alert('Sign In dialog opens here!')}
              variant="secondary"
              size="sm"
            >
              Sign In
            ...
            <div className="px-4 pt-3 pb-6 space-y-3">
              {navLinks.map((link) => (
                <a
                  ...
                  className="block px-4 py-2.5 rounded-xl font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-brand-600 transition-colors"
                >
                  {link.name}
                </a>
              ))}
