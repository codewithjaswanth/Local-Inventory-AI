        ...
        >
          ...
          <div className="p-4 sm:p-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900/80 flex items-center space-x-3">
            ...
            <button
              type="button"
              onClick={onClose}
              className="p-2 rounded-xl text-xs font-semibold text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white hover:bg-slate-200/60 dark:hover:bg-slate-800"
            >
              ESC
            </button>
          </div>
          ...
          <div className="px-6 py-3 bg-white dark:bg-[#060D1A] border-b border-slate-200 dark:border-slate-800 flex items-center space-x-2 overflow-x-auto no-scrollbar text-xs">
            ...
            {['Organic Tomatoes', 'Fresh Croissants', 'Hass Avocados', 'Greek Yogurt'].map((prompt) => (
              <button
                ...
                className="px-2.5 py-1 rounded-full bg-emerald-500/10 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300 hover:bg-emerald-500/20 font-medium whitespace-nowrap transition-colors border border-emerald-500/20 dark:border-emerald-800"
              >
                ✨ {prompt}
              </button>
            ))}
            ...
            {filteredItems.length === 0 ? (
              ...
              filteredItems.map((item) => (
                <div
                  ...
                  className="group flex items-center justify-between p-4 rounded-2xl border border-slate-200 dark:border-slate-700/60 hover:border-emerald-500/50 bg-slate-50/60 dark:bg-slate-800/60 transition-all cursor-pointer shadow-xs hover:shadow-md"
                >
                  <div className="flex items-start space-x-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold text-emerald-700 dark:text-emerald-300 bg-emerald-500/10 dark:bg-emerald-950/80 px-2.5 py-0.5 rounded-full flex items-center border border-emerald-500/20">
